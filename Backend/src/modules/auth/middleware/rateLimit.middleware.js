...
const loginRateLimit = () => {
  return createRateLimiter({
    ...
    max: config.rateLimiting.login?.max || 1000,
...
const progressiveDelay = (attempts) => {
  ...
  const delay = Math.min(maxDelay, Math.pow(2, attempts) * baseDelay);
...
/**
 * Check if IP is from a data center (simplified)
 * In production, use a proper IP intelligence service
 * @param {String} ip - IP address
 * @returns {Boolean} True if IP is from data center
 */
